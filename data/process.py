...
def input_csv_validation(df: pl.DataFrame) -> bool:
    ...
    expected_columns = {
        "Game", "Player", "Expansions", "Birds", "Bonus Cards", 
        "Food on Cards", "Eggs", "Tucked Cards", "Nectar", 
        "Duet Tokens", "Total"
    }
...
if __name__ == "__main__":
    ...
    data_dir = Path(__file__).parent.parent / "data" # assume here that the raw data is present in data
    score_file = data_dir / "raw" / "scores.csv"
    scores = pl.read_csv(score_file)

    assert input_csv_validation(scores), "Input CSV does not contain the expected columns."
